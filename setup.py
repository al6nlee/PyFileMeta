...
SRC_DIR = "py_file_meta"
MODULE_NAME = 'py_file_meta'
VERSION = '0.1.0'
...
class UploadCommand(Command):
    ...
    def run(self):
        ...
        os.system('{0} setup.py bdist_wheel --universal'.format(sys.executable))  # sys.executable 为 Python 解释器的路径

        output('Uploading the package to PyPI via Twine…')
        os.system('twine upload dist/*')

        output('Clean....')
        os.system(f'rm -rf {SRC_DIR}.egg-info dist build')
        sys.exit()
...
if __name__ == '__main__':
    ...
    setup(
        ...
        install_requires=['pyyaml'],  # 项目依赖哪些库(内置库就可以不用写了)，这些库会在pip install的时候自动安装
        python_requires='>=3.8',
